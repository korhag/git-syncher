...
- Closing the window runs one last Refresh, then warns if any repositories are still unsynced. **Stay and sync** cancels close so you can keep working; **Close anyway** exits

## [1.13.4] - 2026-09-06

### Fixed

- Project cards and the project screen no longer paint a huge grey box on small displays (wrapping rows cannot use `expand`)
- Card actions, path, and status stay visible; the project screen shows title, tools, and Commit/Pull/Push instead of a blank page

## [1.13.3] - 2026-09-06

### Fixed

- Add/Edit project **Folder path** can be typed by hand (`~/…` is expanded)
- Browse uses the system folder dialog (Zenity on Linux) so it works on Raspberry Pi / VNC, instead of Flet’s FilePicker which often never opens

## [1.13.2] - 2026-09-06

### Fixed

- Window and dialogs shrink to the monitor (and maximize on compact displays) so a Raspberry Pi or other small screen is no longer cropped
- Unlock, dashboard, and project screens scroll or wrap instead of clipping buttons and form fields

## [1.13.1] - 2026-09-06

### Fixed

- Starting the app no longer crashes with `module 'flet' has no attribute 'run'` when `.venv` was created with Python 3.9 (that Python can only install Flet 0.28)

### Changed

- Install requires Python 3.11+ and Flet 0.80+; Linux/macOS can fetch a local Python 3.12 with uv when the system interpreter is too old
- Flet is installed from PyPI so Raspberry Pi piwheels cannot substitute a broken `flet-web` package

## [1.13.0] - 2026-09-02

### Added

- Add project **From Git** tab clones a remote into an empty folder (account, URL, PAT, and branch), then adds the dashboard card
...
- Adding a folder that is already on the dashboard for that branch **updates** that card instead of blocking with an error

### Fixed

- Account picker fills Git username and email as soon as you select an account
